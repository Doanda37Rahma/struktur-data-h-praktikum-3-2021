...
private:
    ...
    AVLNode* _avl_createNode(char value[LEN]) {
    ...
    AVLNode* _search(AVLNode *root, char value[LEN]) {
        while (root != NULL) {
    ...
    AVLNode* _insert_AVL(AVLNode* node,char value[LEN]) {
    ...
    AVLNode* _remove_AVL(AVLNode* node,char value[LEN]){
        if(node==NULL)
            return node;
        if(strcmp(value , node->data) > 0)
...
public:
    ...
    bool find(char value[LEN]) {
        AVLNode *temp = _search(_root, value);
        if (temp == NULL)
            return false;
        
        if (strcmp(temp->data, value)==0) return true;
        else return false;
    }

    void insert(char value[LEN]) {
        if (!find(value)) {
